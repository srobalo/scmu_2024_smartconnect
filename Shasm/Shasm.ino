...
unsigned long onTime = 5000;  // Time to keep the LED on in milliseconds
...
void loop() {
  ...
  scene();  // Handle client connections and sensor checking
   long currentTime = timeClient.getEpochTime();
  String formattedTime = formatISO8601(currentTime);
  checkSensors(formattedTime);
  updateLEDState(formattedTime);
}
      ...
      if (client.available()) {
        ...
        if (c == '\n') {         // If the byte is a newline character
          ...
          if (currentLine.startsWith("GET /servo/photoresistor/on")) {
            ...
            break;
          } else if (currentLine.startsWith("GET /13/off")) {
...
void checkSensors(String formattedTime) {
  //digitalWrite(ledPin, LOW);
  ...
  Serial.print("ledEnabled with motion: ");
  Serial.println(ledEnabled_motion); 
  Serial.print("ledEnabled with photoresistor: ");
  Serial.println(ledEnabled_photoresistor); 
  Serial.print("Servo enabled with photoresistor: ");
  Serial.println(lightSceneEnabled); 
  Serial.print("Servo enabled with motion sensor: ");
  Serial.println(motionSensorEnabled);
    Serial.print("Light level: "); 
  Serial.println(lightLevel);  // Output current motion reading

  // delay(500);
  if (lightSceneEnabled) {
    if (lightLevel < 1000) {
      /**firebase.setInt("sensorData/photoResistor/id", photoResistorPin);
  ...
  if (motionSensorEnabled && currentMotion == HIGH && previousHistoryClear) {
    ...
    deactivateServo(formattedTime);  // Simulate door opening
    ...
    delay(300);
    activateServo(formattedTime);  // Simulate door closing
    firebase.setInt("sensorData/motionDetected/id", motionSensorPin);
...
void updateLEDState(String formattedTime) {
    unsigned long currentMillis = millis();  // Get the current time

    if (ledEnabled_motion && ledState == HIGH && (currentMillis - previousMillis >= onTime)) {
        // If the LED is on and the onTime has passed, turn it off
        firebase.setInt("sensorData/ledAction/id", ledPin);
    firebase.setInt("sensorData/ledAction/value", 0);
    firebase.setString("sensorData/ledAction/timestamp", formattedTime);
        ledState = LOW;
